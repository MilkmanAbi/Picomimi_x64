    ...
    for (tag = (struct multiboot2_tag *)addr;
        ...
        switch (tag->type) {
            ...
            case MULTIBOOT2_TAG_TYPE_FRAMEBUFFER: {
                ...
                printk(KERN_INFO "Framebuffer: %ux%u@%u at 0x%lx pitch=%u type=%u\n",
                       fb->framebuffer_width, fb->framebuffer_height,
                       fb->framebuffer_bpp, fb->framebuffer_addr,
                       fb->framebuffer_pitch, fb->framebuffer_type);
                /* Store tag for fb_init() after vmm is up */
                kernel_state.fb_tag = (void *)tag;
                break;
            }
...
void __noreturn kernel_main(u32 magic, u64 multiboot_info) {
    ...
    // Initialize framebuffer (needs vmm + slab up for ioremap/kmalloc)
    printk(KERN_INFO "[INIT] Framebuffer driver...\n");
    if (fb_init(kernel_state.fb_tag) == 0) {
        printk(KERN_INFO "[INIT] Framebuffer active - graphics mode\n");
        /* Enable double buffering to eliminate tearing */
        if (fb_backbuffer_init() == 0) {
            fb_set_backbuffer(1);
        }
    } else {
        printk(KERN_WARNING "[INIT] No framebuffer, using VGA text fallback\n");
    }

    // Initialize input event subsystem (before kbd/mouse drivers)
    printk(KERN_INFO "[INIT] Input event subsystem...\n");
    extern void input_init(void);
    input_init();

    // Initialize keyboard driver (replaces stub)
    printk(KERN_INFO "[INIT] Keyboard driver...\n");
    kbd_init();
    ...
    // Create mountpoint stubs so procfs/devfs/sysfs can mount on them.
    // We call into the VFS mkdir directly — current is kernel_main thread.
    {
    ...
    // Initialize character device layer (null, zero, random, kmsg, mem)
    printk(KERN_INFO "[INIT] Character devices...\n");
    extern void chrdev_init(void);
    chrdev_init();

    // Initialize TTY subsystem
    printk(KERN_INFO "[INIT] TTY subsystem...\n");
    ...
    printk(KERN_INFO "[INIT] Pipe filesystem...\n");
    extern int init_pipefs(void);
    init_pipefs();

    // Initialize procfs (/proc)
    printk(KERN_INFO "[INIT] Process filesystem (/proc)...\n");
    extern int init_procfs(void);
    err = init_procfs();
    if (err) {
        printk(KERN_ERR "[INIT] Failed to mount /proc: %d\n", err);
    }

    // Initialize sysfs (/sys)
    printk(KERN_INFO "[INIT] Sysfs (/sys)...\n");
    extern int init_sysfs(void);
    err = init_sysfs();
    if (err) {
        printk(KERN_ERR "[INIT] Failed to mount /sys: %d\n", err);
    }

    // Initialize sysctl parameter store
    printk(KERN_INFO "[INIT] Sysctl...\n");
    extern void sysctl_init(void);
    sysctl_init();

    // Initialize network subsystem
    printk(KERN_INFO "[INIT] Network subsystem...\n");
    ...
    // Start framebuffer console
    extern void fbcon_init(void);
    fbcon_init();

    // Hand off to init (PID 1) via the scheduler.
    // init_task.context was initialised in create_init_task() with rip=init_main,
    // so the first schedule() call context-switches into init_main with a real stack.
    // This CPU becomes the idle task from here on.
    printk(KERN_INFO "[INIT] Handing off to PID 1 via scheduler...\n");
    schedule();

    // Idle loop - we only reach here when no other task is runnable
    while (1) {
        cpu_relax();
        __asm__ volatile("hlt");
        schedule();   // re-check for runnable tasks after each wake
    }
}
