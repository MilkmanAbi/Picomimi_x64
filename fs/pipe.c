...
static void pipe_wake_readers(pipe_inode_t *pi) {
    /* In a full SMP kernel we'd use wait_queue semantics;
     * for now, waking means setting state to RUNNING on blocked
     * tasks. Since we use HLT-based blocking, returning from HLT
     * is sufficient — the read/write loops recheck conditions. */
    (void)pi;
    /* TODO: walk read_wait and wake_up each task */
}

static void pipe_wake_writers(pipe_inode_t *pi) {
    (void)pi;
    /* TODO: walk write_wait and wake_up each task */
}
...
static s64 pipe_write(file_t *file, const char *buf, size_t count, u64 *ppos) {
    ...
    while (written < count) {
        ...
        if (space == 0 || (must_be_atomic && space < count)) {
            ...
            if (nonblock) return written > 0 ? (s64)written : -EAGAIN;

            __asm__ volatile("hlt");

            if (current && signal_pending(current))
                return written > 0 ? (s64)written : -EINTR;
