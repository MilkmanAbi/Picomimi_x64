...
/* Forward declaration of timer_list to avoid circular include */
...
typedef struct vm_area {
    ...
    u64                 pgoff;          // Page offset for file mappings
    struct vm_area      *next;          // Next VMA in list
    struct vm_area      *prev;          // Previous VMA
    struct list_head    vm_list;        // For list_head-based iteration
} vm_area_t;
...
typedef struct sigpending {
    ...
    struct list_head    queue;          // Queue of siginfo (list_head)
} sigpending_t;
...
typedef struct task_struct {
    ...
    pid_t               ppid;           // Parent PID (cached for fast lookup)
    ...
    int                 stop_signal;    // Signal that caused last stop
    bool                wait_reported;  // True if stopped/continued was reported via wait4

    // Alarm timer
    struct timer_list   alarm_timer;    // POSIX alarm() timer
