...
u32 smp_processor_id(void) {
    /* CPUID leaf 1, EBX[31:24] = initial APIC ID of the executing CPU.
     * Safe on ALL x86-64 CPUs and QEMU models. QEMU assigns LAPIC IDs
     * 0,1,2,3... for CPUs 0,1,2,3... so this is always the logical index. */
    u32 eax, ebx, ecx, edx;
...
files_struct_t *files_copy(files_struct_t *old) {
    ...
    new->max_fds = old->max_fds > 64 ? 64 : old->max_fds;
...
void put_unused_fd(int fd) {
    ...
    spin_lock(&files->lock);
    if ((int)fd >= 0 && fd < (unsigned int)files->max_fds) {
...
mm_struct_t *mm_alloc(void) {
    ...
    // pmm_alloc_page() returns a RAW PHYSICAL address.
    // Always access page-table memory via the higher-half alias
    // (phys + KERNEL_PHYS_BASE) so this works regardless of which CR3 is active.
#define KPHYS_ALIAS(phys) ((u64 *)((u64)(phys) + 0xFFFFFFFF80000000ULL))
    phys_addr_t pgd_phys = pmm_alloc_page();
    if (!pgd_phys) {
        kfree(mm);
        return NULL;
    }
    mm->pgd = (u64 *)pgd_phys;   /* stored as raw phys — do NOT dereference directly */

    u64 *pgd_va = KPHYS_ALIAS(pgd_phys);
    memset(pgd_va, 0, 4096);

    // Copy kernel higher-half page tables (entries 256-511).
    // Entry 0 is left as 0 — vmm_map_page will build user page structures there.
    extern u64 *kernel_pml4;
    u64 *kpml4_va = KPHYS_ALIAS(kernel_pml4);
...
mm_struct_t *mm_copy(mm_struct_t *old) {
    ...
    // Copy page mappings: for each VMA, allocate new physical pages
    // and copy the content. Use per-page PTE flags from parent's
    // page tables (not VMA flags) so mprotect'd sub-ranges are honored.
    extern void vmm_map_page_in_pgd(u64 pgd_phys, u64 virt, u64 phys, u64 flags);
    extern u64 vmm_user_virt_to_phys_in(u64 pgd_phys, u64 virt);
    extern u64 vmm_pte_for_virt_in(u64 pgd_phys, u64 virt);

    u64 child_pgd = (u64)new->pgd;
    vm_area_t *vma2 = old->mmap;
    while (vma2) {
        for (u64 pg = vma2->start; pg < vma2->end; pg += 4096) {
            u64 src_pte = vmm_pte_for_virt_in((u64)old->pgd, pg);
            if (!(src_pte & 1)) continue;
            u64 src_phys = src_pte & 0x000FFFFFFFFFF000ULL;
            u64 dst_phys = pmm_alloc_page();
            if (!dst_phys) continue;
            memcpy((void *)(dst_phys + 0xFFFFFFFF80000000ULL),
                   (void *)(src_phys + 0xFFFFFFFF80000000ULL), 4096);
            /* Preserve exact PTE flags from parent (write, nx, user, etc.) */
            u64 pte_flags = src_pte & 0xFFF0000000000FFFULL; /* flags mask */
            vmm_map_page_in_pgd(child_pgd, pg, dst_phys, pte_flags);
        }
        vma2 = vma2->next;
    }
...
task_struct_t *task_create(const char *name, void (*entry)(void *), void *arg, unsigned int flags) {
    ...
    // Signal structures
    task->sighand = kmalloc(sizeof(sighand_struct_t), GFP_KERNEL);
    if (task->sighand) {
        memset(task->sighand, 0, sizeof(sighand_struct_t));
        atomic_set(&task->sighand->count, 1);
    }
    task->signal = kmalloc(sizeof(signal_struct_t), GFP_KERNEL);
    if (task->signal) {
        memset(task->signal, 0, sizeof(signal_struct_t));
        atomic_set(&task->signal->count, 1);
        task->signal->pgrp    = task->pid;
        task->signal->session = task->pid;
    }
    ...
    *stack = (u64)arg;
    ...
    /* Enqueue into the O(1) runqueue so the scheduler can pick it up */
...
task_struct_t *task_fork(unsigned long clone_flags, unsigned long stack_start, 
    ...
    if (!parent) {
        printk(KERN_ERR "[fork] current is NULL!\n");
    ...
    // Set up the child's kernel stack so when it's first scheduled it returns
    // to user space with rax=0 (the fork return value for the child).
    // We push a sc_regs_t-compatible frame onto the child's kernel stack and
    // set context.rip to a trampoline that does RESTORE_REGS + iretq.
    // The frame matches the layout used by syscall_dispatch / SAVE_REGS.
    ...
    // Get the parent's user return context from the saved syscall frame
    u64 user_rip = 0, user_rsp_val = 0, user_rflags = 0x202;
    if (parent->trap_frame) {
        // trap_frame_t layout: ... rcx=user_rip at rcx offset, rip field = user RIP
        user_rip    = parent->trap_frame->rip;
        user_rsp_val = parent->trap_frame->rsp;
        user_rflags  = parent->trap_frame->rflags;
        printk(KERN_DEBUG "[fork] trap_frame: rip=%llx rsp=%llx rbp=%llx r15=%llx\n",
               user_rip, user_rsp_val, parent->trap_frame->rbp, parent->trap_frame->r15);
    } else {
        printk(KERN_ERR "[fork] parent has NO trap_frame!\n");
    }
    if (stack_start) user_rsp_val = stack_start;

    // Push frame onto child's kernel stack (stack grows down)
    u64 kstack_top = (u64)child->stack;
    kstack_top -= sizeof(fork_sc_frame_t);
    fork_sc_frame_t *frame = (fork_sc_frame_t *)kstack_top;
    memset(frame, 0, sizeof(*frame));

    // Copy all saved user registers from parent's syscall trap frame.
    // IMPORTANT: trap_frame is cast from sc_regs_t* which has layout:
    //   rax,rbx,rcx,rdx,rsi,rdi,rbp,r8,r9,r10,r11,r12,r13,r14,r15
    //   (rax at lowest address = rsp after SAVE_REGS).
    // trap_frame_t has the same fields but WRONG order (r15 first) so
    // we MUST use the sc_regs layout here.
    typedef struct __packed {
        u64 _rax, _rbx, _rcx, _rdx, _rsi, _rdi, _rbp;
        u64 _r8, _r9, _r10, _r11, _r12, _r13, _r14, _r15;
        u64 _vector, _error_code;
        u64 _rip, _cs, _rflags, _user_rsp, _ss;
    } sc_frame_t;
    if (parent->trap_frame) {
        sc_frame_t *tf = (sc_frame_t *)parent->trap_frame;
        frame->rax = 0;             // child fork return = 0
        frame->rbx = tf->_rbx;
        frame->rcx = tf->_rcx;
        frame->rdx = tf->_rdx;
        frame->rsi = tf->_rsi;
        ...
        frame->rip      = tf->_rip;
        frame->cs       = tf->_cs;
        frame->rflags   = tf->_rflags | 0x200;
        frame->user_rsp = stack_start ? stack_start : tf->_user_rsp;
        frame->ss       = tf->_ss;
    } else {
        frame->rax      = 0;
        frame->rip      = user_rip;
        frame->cs       = 0x2B;
        frame->rflags   = user_rflags | 0x200;
        frame->user_rsp = user_rsp_val;
        frame->ss       = 0x33;
    }
    frame->vector     = 0x80;
    frame->error_code = 0;

    // Point child's kernel context to the fork_return trampoline
    extern void fork_return_to_user(void);
    child->context.rsp = kstack_top;
    child->context.rip = (u64)fork_return_to_user;
...
void wake_up_process(task_struct_t *task) {
    if (!task) return;
    task->state = TASK_RUNNING;
    /* Delegate to the real scheduler to enqueue on the correct runqueue */
    extern void sched_enqueue_task(task_struct_t *t);
    sched_enqueue_task(task);
}
...
void schedule(void) {
    /* Delegate to the O(1)/CFS hybrid scheduler in sched.c */
    extern void schedule_impl(void);
    schedule_impl();
}
...
task_struct_t *create_init_task(void) {
    ...
    // CRITICAL: initialize the saved context so context_switch() can safely
    // restore into init_task.  Without this, context.rsp == 0 and the very
    // first context switch causes a triple fault (observed on VirtualBox).
    //
    // We point rip at init_main and rsp at the top of the freshly-allocated
    // kernel stack.  context_switch() will load these on the first switch,
    // landing in init_main with a valid stack.
    extern void init_main(void);
    init_task.context.rip    = (u64)init_main;
    init_task.context.rsp    = (u64)init_task.stack - 8; // standard ABI alignment
    init_task.context.rflags = 0x200; // IF=1, rest default

    task_table[1] = &init_task;

    /* Enqueue PID 1 into the O(1) scheduler runqueue */
    extern void sched_enqueue_task(task_struct_t *t);
    sched_enqueue_task(&init_task);
...
/* null_fops — no-op file operations for epoll fd stubs */
static s64 null_read(file_t *f, char *b, size_t n, u64 *pos) { (void)f;(void)b;(void)n;(void)pos; return 0; }
static s64 null_write(file_t *f, const char *b, size_t n, u64 *pos) { (void)f;(void)b;(void)n;(void)pos; return (s64)n; }
const file_operations_t null_fops = {
    .read  = null_read,
    .write = null_write,
};
