...
int do_execve(const char *filename, char *const argv[], char *const envp[]) {
    ...
    /* ================================================================
     * Shebang (#!) interpreter script detection
     * Must be checked BEFORE ELF/Mach-O, because a script is neither.
     * ================================================================ */
    {
        char magic[256];
        u64 pos = 0;
        s64 nr = vfs_read(file, magic, sizeof(magic) - 1, (s64*)&pos);
        if (nr >= 2 && magic[0] == '#' && magic[1] == '!') {
            magic[nr] = '\0';
            /* Find end of first line */
            char *eol = magic + 2;
            while (*eol && *eol != '\n' && *eol != '\r') eol++;
            *eol = '\0';
            /* Parse: #!/path/to/interp [optional-arg] */
            char *interp = magic + 2;
            ...
            /* Build new argv: [interp, iarg?, script, original_args...] */
            int new_argc = 0;
            char **new_argv = kmalloc((size_t)(argc + 4) * sizeof(char *), GFP_KERNEL);
            if (!new_argv) { ret = -ENOMEM; goto err_free; }

            /* interp */
            size_t ilen = strlen(interp) + 1;
            new_argv[new_argc] = kmalloc(ilen, GFP_KERNEL);
            if (new_argv[new_argc]) { memcpy(new_argv[new_argc], interp, ilen); new_argc++; }

            /* optional arg */
            if (iarg) {
                size_t alen = strlen(iarg) + 1;
                new_argv[new_argc] = kmalloc(alen, GFP_KERNEL);
                if (new_argv[new_argc]) { memcpy(new_argv[new_argc], iarg, alen); new_argc++; }
            }

            /* script path (k_filename) */
            size_t flen = strlen(k_filename) + 1;
            new_argv[new_argc] = kmalloc(flen, GFP_KERNEL);
            if (new_argv[new_argc]) { memcpy(new_argv[new_argc], k_filename, flen); new_argc++; }

            /* original args (skip argv[0]) */
            for (int i = 1; i < argc; i++) {
                size_t al = strlen(k_argv[i]) + 1;
                new_argv[new_argc] = kmalloc(al, GFP_KERNEL);
                if (new_argv[new_argc]) { memcpy(new_argv[new_argc], k_argv[i], al); new_argc++; }
            }
            new_argv[new_argc] = NULL;

            /* Free old k_argv/k_envp — we'll re-exec with new ones */
            free_strarray(k_argv, argc);

            /* Recursive execve with the interpreter */
            char interp_copy[256];
            strncpy(interp_copy, interp, 255); interp_copy[255] = '\0';

            /* do_execve will re-copy new_argv */
            ret = do_execve(interp_copy, (char *const *)new_argv, (char *const *)envp);

            /* Free new_argv */
            for (int i = 0; i < new_argc; i++) kfree(new_argv[i]);
            kfree(new_argv);
    ...
    /* Re-open: position was advanced by the magic read above */
    filp_close(file, NULL);
    file = filp_open(k_filename, O_RDONLY, 0);
    if (!file || IS_ERR(file)) { ret = -ENOENT; goto err_free; }
